class Node:
    def __init__(self, key_, data_):
        ...
        self.right = None
        self.height = 1
#trzeba będzie zaimplementować obrt tak, żeby różnica długości poddrzew nie była większa niż 1
...
class Tree:
    ...
    def insert(self, key, data):
        ...
        #do tutaj stary kod, wstawiam Node na jego miejsce
        currentNode = self.__insert(self.root, key, data)
        #aktualizuję jego wysokość
        currentNode.height = 1+max(self.height(currentNode.left), self.height(currentNode.right))

        #tutaj zaczynam balansować jeśli różnica poddrzew jest większa od 1
        #4 przypadki jakie mogą się pojawić
        balance = self.balance()

        #LL 
        if  balance == -2 and key < currentNode.left.key:
            return self.rotate_right()
        #LR
        if balance == -2 and key > currentNode.left.key:
            currentNode = self.rotate_left()
            self.rotate_right()
        #RL
        if balance == 2 and key < currentNode.right.key:
            self.rotate_left()
        #RR
        if balance == 2 and key > currentNode.right.key:
    ...
    #funkcja sprawdzająca różnicę wysokości poddrzew lewe-prawe więc jak wyjdzie ujemne znaczy że prawe jest większe
    def balance(self):
        return self.__height(self.root.right) - self.__height(self.root.left)
    
    #zakładam sytuację x, y, z to ojciec, syn i wnuk. Na koniec to syn ma być ojcem 
    def rotate_right(self, x):
        #ustalam dziecko i wnuka
        y = x.left
        z = y.right
        #obracam w prawo
        y.right = x
